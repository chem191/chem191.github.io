...
![<small>*Your final setup should look something like this picture.  Red = 5V, black = GND, and blue = signal.  *</small>](images/breadboard_thermistor_teensy.png)
...
Where $T$ is temperature, $T_0$ is room temperature (in K), $B$ is an empiracal value unique to each type of thermistor (3977 in for the NTCLE100E3103*B0 thermistor we're using, as given on the datasheet), $R$ is the resistance of the thermistor at temperature $T$, and $R_0$ is the resistance of the thermistor at room temperature, $T_0$ (10 k$\Omega$ for this thermistor, as specified by the datasheet).

1. The first step toward printing a temperature is to convert from bits (`sensorValue`) to resistance, $R$. Use the following code to do this:

```{Rcpp, eval = FALSE}
    void loop() {
        int value = analogRead(0); // read sensor
        float resistance = 8192.0 / value - 1.0;
    }
```

    > *The trailing zeroes (after the decimal points) ARE important in this case!*
...
```
    ...
    > *Notice that you can use Arduino as a calculator!*
...
1. Once you've converted to C and F, print the results for both degrees C and degrees F.  Your values should:
...
1. As a last step, use a multimeter to measure the resistance of the blue resistor you used.  Substitute this value for the `10.0` in the temperature calculation and see what happens to your percent error.
