...
**To turn in:** Turn in your worksheet by the end of class.
...
\[
    ^{\circ}\text{C} = 25 + (\text{value} - 512) / 11.3
\]
...
```{Rcpp, eval = FALSE}
    void loop() {
        value = analogRead(0); // read sensor
        temp  = ____________________________; // convert to temperature
        ______________________; // print the temperature to the serial port.
        delay(1000);            // wait for 1 second
    }
```

2. Upload your code and see if it works.
...
1. Add a line just below `Serial.print(temp)` to print the "*C" symbology:
...
1. Once you've converted to F, print the results for both degrees C and degrees F.  Your values should:
